...
int main(int argc, char** argv) {
    ...
    std::ofstream ofs{output_folder.file_path("snapshot_run.csv")};
    ofs << std::setprecision(10);

    for(std::size_t ss = 0; ss<=sanapshot_samples; ss++) {

        auto coeff = static_cast<double>(ss)/sanapshot_samples;
        auto grasp_start = std::chrono::high_resolution_clock::now();

        ctop::Grasp grasp;
        grasp.max_threads = std::numeric_limits<int>::max();
        grasp.max_iterations = 10;
        grasp.max_did_not_improve = 10;
        grasp.snapshot_coef = coeff;
        ...
        ofs << coeff << "," << grasp.duration_raw.count() << "," << g->sol.reward << "\n";
