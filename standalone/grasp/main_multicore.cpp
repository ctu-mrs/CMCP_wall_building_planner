...
bool
isPointInside(double px, double py, const std::vector<ctop::Point3D>& corners) {
    int count = 0;
...
double solve_closest_points_on_planes(ctop::Plane& b1, ctop::Plane& b2) {
    ...
    //CTOP_LOG_E("[u v] = [{}, {}], [u' v'] = [{}, {}]", uv(0), uv(1), uv_prime(0), uv_prime(1));
    //CTOP_LOG_E("[u v] = [{}, {}], [u' v'] = [{}, {}]", x(0), x(1), x(2), x(3));
...
CloseWallInfo find_closest_planes_top_bottom(ctop::Brick& b1, ctop::Brick& b2) {
    ...
    for(int i = 4; i != 6; ++i) {
        ...
        for(int j = 4; j != 6; ++j) {
            ...
            //CTOP_LOG_E("test b1[id={}] wall={} b2[id={}] wall={} dist={}", b1.id, ctop::BrickWalls::wall_pos2string(i), b2.id, ctop::BrickWalls::wall_pos2string(j), dist);
...
void eval_distances(std::vector<ctop::Brick>& bricks) {
    for(int i = 0; i != bricks.size(); ++i) {
        ...
        for(int j = i+1; j != bricks.size(); ++j) {
            ...
            if(info.dist <= 0.11) {
                hasSide = true;
                CTOP_LOG_I("min_plane b1[id={}]: {}, b2[id={}]: {}, dist: {}", b1.id, ctop::BrickWalls::wall_pos2string_set(info.i), b2.id, ctop::BrickWalls::wall_pos2string_set(info.j), info.dist);
            } else {
                //CTOP_LOG_D("min_plane b1[id={}]: {}, b2[id={}]: {}, dist: {}", b1.id, ctop::BrickWalls::wall_pos2string_set(info.i), b2.id, ctop::BrickWalls::wall_pos2string_set(info.j), info.dist);
            }
...
int main(int argc, char** argv) {
    ...
    if(!config.problemFileObj.empty()) {
        ...
        ctop::parse_csv_line(problem_file, [&](int line, const std::vector<std::string>& vals){
            ...
            if(parse_brick) {
                ...
                b.reward = wall.brick_rewards_map[b.name];
                b.duration = wall.brick_durations_map[b.name];
                b.req_robots = wall.brick_num_robot_requirements_map[b.name];
                b.reservoir_time = wall.brick_reservoir_time_from_wall_s_map[b.name];
    ...
    for(int i = 0; i<b_size; i++) {
        ...
        for(int j = i+1; j<b_size; j++) {
            ...
            if(std::abs(std::abs(dz) - 0.2) < 0.01) {
                ...
                if(wall.entire_layer_first || ctop::Brick::is_on_top_of_each_other(&bottom_brick, &top_brick)) {
                    graph.add_dir_edge(bottom_brick, top_brick, ctop::WallEdgeType::precedence);
                }

            }
            if(std::abs(dz) < 0.01) {
                graph.add_both_dir_edge(b1, b2, ctop::WallEdgeType::side_activate);
                auto layer = static_cast<int>(std::floor((std::abs(b1.z) - 0.1)/0.2 + 0.5));
                if(layer < 0) layer = 0;
                if(b1.layer == 0) b1.layer = layer;
                if(b2.layer == 0) b2.layer = layer;
    ...
    std::unordered_map<uint64_t, std::vector<std::shared_ptr<ctop::Resource>>> resource_bucket;
    int rid = 0;
    ...
    const std::size_t sanapshot_samples = 10;

    std::ofstream ofs;
    // white to the end of file
    ofs.open(output_folder.file_path("multi_core_run.csv"), std::ios::app);
    ofs << std::setprecision(10);

    std::vector<int> numcores{1,2,4,8,12,16,20};
    for(auto&& cores : numcores) {

        auto grasp_start = std::chrono::high_resolution_clock::now();

        ctop::Grasp grasp;
        grasp.max_threads = cores; std::numeric_limits<int>::max();
        grasp.max_iterations = 100;
        grasp.max_did_not_improve = 100;
        grasp.snapshot_coef = 0.5;
        ...
        auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(grasp.duration_raw).count();
